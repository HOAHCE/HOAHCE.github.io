---
layout: course
title: "Khởi nghiệp và đổi mới sáng tạo"
description: "Hình thành tư duy khởi nghiệp và đổi mới sáng tạo: nhận diện cơ hội, xây dựng mô hình kinh doanh, kiểm chứng ý tưởng và trình bày dự án."
title_en: "Entrepreneurship and Innovation"
description_en: "Entrepreneurial and innovation thinking: spotting opportunities, building a business model, validating ideas and pitching a venture."
instructor: Trần Thái Hòa
year: 2025
course_id: khoi-nghiep-doi-moi-sang-tao
# Muốn hiện thêm thông tin lớp thì bỏ dấu # và điền vào:
# term: "Học kỳ I"            # term_en: "Semester 1"
# location: "Phòng ..."
# time: "Thứ Hai, tiết 1–3"

schedule:
  - week: 1
    topic: "Chương 1 — Xây dựng ý tưởng kinh doanh"
    description: "Nguồn gốc ý tưởng, phát biểu vấn đề, kỹ thuật HMW và SCAMPER, ma trận sàng lọc có trọng số, viết giả thuyết kèm ngưỡng kiểm chứng."
    materials:
      - name: Slide Chương 1 (62 slide, PDF)
        url: /assets/pdf/knst/Chuong-1-Xay-dung-y-tuong-kinh-doanh.pdf
  - week: 2
    topic: "Chương 2 — Phân khúc khách hàng và chân dung khách hàng"
    description: "Phân khúc và chọn phân khúc đầu tiên, phỏng vấn theo Mom Test, bản đồ đồng cảm, dựng persona, ước lượng TAM – SAM – SOM từ dưới lên."
    materials:
      - name: Slide Chương 2 (52 slide, PDF)
        url: /assets/pdf/knst/Chuong-2-Phan-khuc-khach-hang.pdf
  - week: 3
    topic: "Chương 3 — Mô hình kinh doanh với Business Model Canvas"
    description: "Chín ô của canvas và bốn phép kiểm tra nhất quán, tuyên bố giá trị, kinh tế đơn vị: biên gộp, điểm hòa vốn, CAC, LTV và tỷ lệ LTV/CAC."
    materials:
      - name: Slide Chương 3 (49 slide, PDF)
        url: /assets/pdf/knst/Chuong-3-Business-Model-Canvas.pdf
  - week: 4
    topic: "Chương 4 — Pitching gọi vốn"
    description: "Cấu trúc bài pitch, quy tắc 10–20–30, định giá trước và sau đầu tư, cách trả lời câu hỏi khó theo công thức Thừa nhận – Dữ liệu – Kế hoạch."
    materials:
      - name: Slide Chương 4 (49 slide, PDF)
        url: /assets/pdf/knst/Chuong-4-Pitching-goi-von.pdf
  - week: 5
    topic: "Case study NấmNhà và tổng duyệt"
    description: "Mổ xẻ một dự án đi trọn bốn chương, đối chiếu với dự án MộcLoop, sau đó các nhóm tổng duyệt chéo theo đúng rubric của buổi Demo Day."
    materials:
      - name: Slide Buổi 5 (43 slide, PDF)
        url: /assets/pdf/knst/Buoi-5-Case-study-NamNha.pdf
  - week: 6
    topic: "Demo Day"
    description: "Các nhóm trình bày dự án trước hội đồng, hỏi đáp và công bố kết quả. Slide dưới đây là kịch bản điều hành dùng trên sân khấu."
    materials:
      - name: Slide Demo Day (39 slide, PDF)
        url: /assets/pdf/knst/Demo-Day-Kich-ban-dieu-hanh.pdf
---

## Giáo trình

Giáo trình của học phần được biên soạn theo nguyên tắc **chơi trước – hiểu sau – làm ngay**. Mỗi chương gồm phần lý thuyết cô đọng vừa đủ để cả lớp có ngôn ngữ chung, một trò chơi mô phỏng buộc sinh viên ra quyết định dưới sức ép thời gian và nguồn lực hữu hạn, phần giải mã biến trải nghiệm thành quy luật, và hệ thống bài tập đưa quy luật ấy vào chính dự án của nhóm.

Điểm khác biệt so với tài liệu khởi nghiệp thông thường: sinh viên không học về một dự án giả định của người khác mà xây dựng dự án của chính mình xuyên suốt năm chương. Đầu ra của chương trước là đầu vào bắt buộc của chương sau — phát biểu vấn đề ở Chương 1 được kiểm chứng bằng phỏng vấn ở Chương 2, chuyển thành mô hình kinh doanh ở Chương 3 và trở thành bài pitch gọi vốn ở Chương 4. Đến buổi cuối, mỗi nhóm có một hồ sơ dự án hoàn chỉnh chứ không phải một tập bài tập rời rạc.

<a href="/assets/pdf/knst/Giao_trinh_Khoi_nghiep_Doi_moi_Sang_tao.pdf" class="btn btn-sm z-depth-0" role="button" target="_blank" rel="noopener noreferrer">Tải giáo trình (PDF, 72 trang)</a>
<a href="/assets/pdf/knst/Giao_trinh_Khoi_nghiep_Doi_moi_Sang_tao.docx" class="btn btn-sm z-depth-0" role="button">Bản Word (.docx)</a>

## Cách sử dụng tài liệu

- **Trước mỗi buổi học:** đọc phần lý thuyết của chương tương ứng trong giáo trình. Slide chỉ là bản tóm lược dùng trên lớp, không thay thế giáo trình.
- **Sau mỗi buổi học:** hoàn thành bài tập cuối chương và bổ sung vào hồ sơ dự án của nhóm. Hồ sơ này là căn cứ chấm điểm ở buổi Demo Day.
- **Trò chơi trong lớp** dùng đồng tiền quy ước Xu Khởi Nghiệp (XU). Nhóm nào tiêu hết vốn trước khi kiểm chứng được nhu cầu sẽ thấy rõ cái giá của việc xây trước, hỏi sau.
- Phát hiện lỗi hoặc có góp ý về tài liệu, sinh viên gửi email cho giảng viên để cập nhật ở bản sau.

Tài liệu tiếp tục được cập nhật trong quá trình giảng dạy. Bản hiện tại: tháng 9 năm 2026.
